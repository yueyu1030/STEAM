...
from model_path import model#,model2
from layers_path import Transformer
from torch.autograd import Variable
import argparse
import json
from utils_path import *
from test_path import testing, testing_f1
...
parser.add_argument('--taxi_feature', type=int, default=1, help='use attn or not in lstm')
    ...
    for i in range(int(len(taxo)/batchsize)):
        ...
        for j in range(i*batchsize, min((i+1)*batchsize, len(taxo)-1)):
            ...
            for pos_sample in taxo[j]["pos"]:
                ...
                embed,_ = models(target_path = target_path, inner_path = inner, inner_freq = freq, attach = int(pos_sample), \
                    ...
                    for y in neg_path:
                        if y == None:
                            ...
                            if len(dep_path[y[0]]["path"])>100: # sample 50 paths if there is too much dependency path
                                ...
                                dependency_paths.append(dep_path[y[0]]["path"])
                                dependency_freq.append(dep_path[y[0]]["freq"])
                    embed , _ = models(target_path = target_path, inner_path = inner, inner_freq = freq, attach = int(neg_sample), \
                    attach_path = dependency_paths, attach_freq = dependency_freq, device = device, compound = compound)
                    #print(embed)
                    #embed = F.softmax(embed, dim = 1)
                    #print(embed)
            ...
            #if j == min((i+1)*batchsize, len(taxo)-1) - 1:
            #    print(labels, pred)
            ...
            optimizer.step()
            pbar.update(1)
            pbar.set_description('Dep %s Enc %s Epoch %d/%d Loss: %.4f' % (args.encode_dep, args.encode_prop, epoch+1, \
             int(args.epochs), running_loss/running_num))
            if epoch % 10 == 0:
...
if __name__ == "__main__":
    ...
    model = model(args = args, path_length = args.path_len, hidden_size = args.hidden, num_layers = 1, num_terms = w2v.shape[0],\
     parameters = parameters, embed_dim= embed_dim, dropout = args.dropout, bidirection = False, taxi_feature = args.taxi_feature) #, attach_path= args.attach_path)
    ...
    with tqdm(total=args.epochs * int(len(taxo)/batchsize)) as pbar:
        for epoch in range(args.epochs):
            if epoch % 5 == 4:
                ...
                #testing(models = model, taxo_test = taxo_test, device = args.cudaid, test_syn = [], test_hyn = test, \
                ...
                testing_f1(models = model, taxo_test = taxo_test, device = args.cudaid, test_syn = [], test_hyn = test, dep_path = paths, \
                    epoch = epoch, args = args,  inv_paths_index = inv_paths_index, idx_cnt = idx_cnt, metric= 'mean', is_softmax = False, \
                    compound = False, synonym = True, innerpath = False)

            model.train()
            train(models = model, taxo = taxo, device = args.cudaid, optimizer = optimizer, batchsize = 10, dep_path = paths)
