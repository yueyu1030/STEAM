...
parser.add_argument('--path_len', type=int, default=3, help='Length of mini-paths')
parser.add_argument('--fp', type=str, default='../data_train_test_path', help='Path for files')
...
parser.add_argument('--lambda1', type=float, default=0.2, help='weight for loss term 1')
parser.add_argument('--lambda2', type=float, default=0.2, help='weight for loss term 2')
parser.add_argument('--load_gcn', type=int, default=0, help='use gcn or not')
parser.add_argument('--load_score', type=int, default=1, help='use gcn score or not')
...
def train(models, taxo, batchsize, dep_path, device, optimizer, neg_sample_ratio = 1, compound = False, synonym = True, innerpath = False):
    ...
    for i in range(int(len(taxo)/batchsize)):
        ...
        for j in range(i*batchsize, min((i+1)*batchsize, len(taxo)-1)):
            ...
            for neg_sample in neg_samples:
                ...
                for y in neg_path:
                    if y == None:
                        ...
                        #if len(y)>1:
                        #    print(len(y),y)
                        if len(dep_path[y[0]]["path"]) > 500: # sample 50 paths if there is too much dependency path
                            sample_idx, sample_freq = random_choice(dep_path[y[0]]["freq"], max_val= 500)
                            #print(dep_path[xx]["path"], dep_path[xx]["freq"], sample_idx, sample_freq)
                            dependency_paths.append([dep_path[y[0]]["path"][x] for x in sample_idx])
                            dependency_freq.append(sample_freq)
                        else:
            ...
            try:
                l += loss(pred, labels) + args.lambda1 * (loss(embed_preds, labels) + loss(paths_preds, labels) + loss(taxi_preds, labels)) \
        ...
        for epoch in range(args.epochs):
            if epoch % 5 == 4:
                ...
                testing_f1(models = model, taxo_test = taxo_test, device = args.cudaid, test_syn = [], test_hyn = test, dep_path = paths, \
                    epoch = epoch, args = args,  inv_paths_index = inv_paths_index, metric= 'mean', is_softmax = True, \
